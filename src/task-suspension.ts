 ...
 * Task Suspension
 *
 * Auto-suspend scheduled tasks when repeated quota/auth errors are detected.
 * Uses exponential backoff and retry-after parsing to determine suspension
 * duration.
 *
 * Ported from EJClaw — adapted for NanoClaw's DB schema (uses status='paused'
 * + next_run for resume scheduling since NanoClaw lacks a suspended_until column).
 */

import { TIMEZONE } from './config.js';
import { getRecentTaskErrors, updateTask } from './db.js';
...
export function evaluateTaskSuspension(
  ...
  const recentErrors = getRecentTaskErrors(
    task.id,
 ...
 *
 * NanoClaw adaptation: sets status to 'paused' and next_run to the resume
 * time. The scheduler's getDueTasks() only picks up status='active' tasks,
 * so this effectively suspends the task. Call resumeTask() to reactivate.
 */
export function suspendTask(taskId: string, suspendedUntil: string): void {
  updateTask(taskId, { status: 'paused', next_run: suspendedUntil });
  logger.info(
...
/**
 * Resume a previously suspended task by setting it back to active.
 */
export function resumeTask(taskId: string): void {
  updateTask(taskId, { status: 'active' });
  logger.info({ taskId }, 'Suspended task resumed');
}

/**
 * Format the suspension notification for messaging channels.
 */
